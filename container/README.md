...
**MCP server merge (same opt-in path)**: dropping the user tier from the settings cascade with `--setting-sources project,local` also drops any `mcpServers` block the operator put in `~/.claude/settings.json`, AND the operator may have additional MCP servers in `~/.claude.json` (where `claude mcp add ...` writes by default — a separate Claude Code config file). To preserve both sources inside the container, `generate-hooks-shim-settings` ALSO reads `~/.claude.json`'s top-level `mcpServers` block (`--mcp-input ~/.claude.json`) and merges it with the settings-side `mcpServers` into the shim output. Settings-side entries win on key collision (matching Claude Code's own user-tier > .claude.json precedence). Every server's `command` field is wrapped with `/usr/local/bin/exec-hook` so the same magic-byte-sniffing logic applies: ELF / shebang-script servers (node, python, Linux binaries) run normally; Mach-O / unknown-format servers silently no-op (the server fails to start but no "Exec format error" spam on every invocation). If a Mac-native MCP server is required at runtime, build a Linux-compatible replacement or accept that it won't connect inside the container. Neither host file is mutated on disk.
