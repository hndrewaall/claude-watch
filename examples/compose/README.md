...
**Trust profile** (`CW_PROFILE`): set `CW_PROFILE=corp-dev-trusted` in the launcher's shell env to opt into a wider default allow-list that adds host-scheduling tooling — `crontab` (Linux + macOS), `launchctl` (macOS launchd), `systemctl` (Linux systemd user units), `schtasks` / `powershell` / `pwsh` (Windows Task Scheduler), `uname` / `sw_vers` / `lsb_release` (host-OS detection), and small filesystem creation tools (`tee`, `mkdir`, `chmod`, `cp`, `mv`, `rm`, `curl`). Default unset (`corp-dev`) keeps the conservative read-only set. Use this when you want the in-container claude to wire periodic claude-event jobs on the host (cron / launchd / systemd timers / Task Scheduler) — see the "Host-side scheduled tasks" section in `container/baked-CLAUDE.md` for the workflow. Operator's explicit `ALLOWED_COMMANDS` in `~/.config/claude-container/mcp-host-bash.env` always wins over the profile default.
